...
print("A_dense sum:", A_dense.sum(axis=1))
...
for exp_id in range(10):
    ...
    propensity = np.divide(np.exp(p1), np.exp(p1) + np.exp(p0))
     ...
     #   print("p1 shape:", p1.shape)
      #  print("p0 shape:", p0.shape)
       # print("propensity shape:", propensity.shape)
    ...
    # simulate treatments
    #print("propensity:", type(propensity))
    T = np.random.binomial(1, p=propensity)
    T_list = list(T)
    print("treatment个数：",T_list.count(1))
    #if exp_id == 0:
    ...
    # simulate outcomes
    weight = np.random.uniform(0,0.8,(X.shape[0],X.shape[0]))
    Y1 = C * (p1 + p0) + epsilon + np.matmul(weight*A_dense,propensity)
    Y0 = C * (p0) + epsilon + np.matmul(weight*A_dense,propensity)
    print("np.matmul(weight*A_dense,propensity):",np.matmul(weight*A_dense,propensity))
    #Y1 = C * (p1 + p0) + epsilon
    #Y0 = C * (p0) + epsilon
    #print("Y1:",Y1.shape)
    ...
    #print("topics_100:", topics_100_dims, topics_100_dims.shape)
    # then we get a union of all those top 100 words
    unique_100_dims = np.unique(topics_100_dims)
    print("unique_100_dim:", unique_100_dims, unique_100_dims.shape)

    # reduce the dimensions by extract the selected words
    X_100 = X[:, unique_100_dims]
    print("X_100:", X_100.shape)

    # X_100.shape

    # save the data
    sio.savemat('./datasets_3/' + name + '0.2'+ '/' + name + str(exp_id) + '.mat', {
        'X_100': X_100, 'T': T, 'Y1': Y1, 'Y0': Y0,
        'Network': data['A'][0][0]})
